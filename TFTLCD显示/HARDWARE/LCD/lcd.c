...
//LCD的画笔颜色和背景色	   
u16 POINT_COLOR=0x0000;	//画笔颜色-黑
u16 BACK_COLOR=0xFFFF;  //背景色-白
...
{   
	LCD->LCD_REG=regval;//写入要写的寄存器序号	 由FSMC控制赋值
}
//写LCD数据
//data:要写入的值
...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	// 根据手册得知，首次得到的是dummy,无效数据-丢弃
 	r = LCD_RD_DATA();						
	
	// 对于1963来说 首次独到的就是有效数据
	if(lcddev.id==0X1963) return r;									// 1963直接读就可以 
	
	// 对于首次无效的情况，需要在延迟2us 继续读，从而读到r
	opt_delay(2);	  
 	r = LCD_RD_DATA();  		  		// 包含 r + g 需要在读一次 才能读到b
	
 	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510)		//9341/NT35310/NT35510要分2次读出
 	{
		opt_delay(2);	  
		b=LCD_RD_DATA(); // 得到b+下一个rgb颜色的分量r
		g=r&0XFF;		//对于9341/5310/5510,第一次读取的是RG的值,R在前,G在后,各占8位
		g<<=8;
		//颜色分量 r g b 的值，分别放置在变量r g b 的高位
	} 
	// 直接返回r
...
void LCD_DrawPoint(u16 x,u16 y)
{
	...
	LCD->LCD_RAM=POINT_COLOR; // 设置画笔颜色
}
...
{ 					
  ...
  // PORTG12复用推挽输出 A0	
	...
	// 配置读时序-用于初始化FSMC
	readWriteTiming.FSMC_AddressSetupTime = 0x01;	 				// 地址建立时间（ADDSET）为2（1+1）个HCLK 1/36M=27ns  
  readWriteTiming.FSMC_AddressHoldTime = 0x00;	 				// 地址保持时间（ADDHLD）模式A未用到	
  readWriteTiming.FSMC_DataSetupTime = 0x0f;		 				// 数据保存时间为16个HCLK,因为液晶驱动IC的读数据的时候，速度不能太快，尤其对1289这个IC。
  readWriteTiming.FSMC_BusTurnAroundDuration = 0x00;		// 无用-暂不配置
  readWriteTiming.FSMC_CLKDivision = 0x00;							// 无用-暂不配置
  readWriteTiming.FSMC_DataLatency = 0x00;							// 无用-暂不配置
  readWriteTiming.FSMC_AccessMode = FSMC_AccessMode_A;	// 模式A 
    
	// 配置写时序-用于初始化FSMC
	writeTiming.FSMC_AddressSetupTime = 0x00;			 				// 地址建立时间（ADDSET）为1个HCLK  0+1
  writeTiming.FSMC_AddressHoldTime = 0x00;			 				// 地址保持时间
  writeTiming.FSMC_DataSetupTime = 0x03;		 		 				// 数据保存时间为4个HCLK	3+1
  writeTiming.FSMC_BusTurnAroundDuration = 0x00;				// 无用-暂不配置
  writeTiming.FSMC_CLKDivision = 0x00;									// 无用-暂不配置
  writeTiming.FSMC_DataLatency = 0x00;									// 无用-暂不配置
  ...
  FSMC_NORSRAMInitStructure.FSMC_WaitSignalPolarity = FSMC_WaitSignalPolarity_Low;
	FSMC_NORSRAMInitStructure.FSMC_AsynchronousWait = FSMC_AsynchronousWait_Disable;  // 是否同步  我们用的是异步
  ...
  FSMC_NORSRAMInitStructure.FSMC_ReadWriteTimingStruct = &readWriteTiming; 		// 赋值读写时序
  FSMC_NORSRAMInitStructure.FSMC_WriteTimingStruct = &writeTiming;  					// 赋值写时序
...
//num:要显示的字符:" "--->"~"   空格到波浪线
//size:字体大小 12/16/24
//mode:叠加方式(1)还是非叠加方式(0) 区别:底色是否保持
void LCD_ShowChar(u16 x,u16 y,u8 num,u8 size,u8 mode)
{  							  
  u8 temp,t1,t;
	u16 y0 = y; // 保存Y轴坐标
	u8 csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数	
 	num=num-' ';// 得到索引-得到偏移后的值（ASCII字库是从空格开始取模，所以-' '就是对应字符的字库） 
